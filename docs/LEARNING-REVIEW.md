...
> 复盘日期：2026-09-01
>
> 当前进度：已完成 Lesson 01 - Lesson 32；Lesson 33 进行中
>
> 当前阶段：针对性补强类型流，随后进入独立项目
...
Lesson 33 已按这个目标设置为订单查询练习。当前没有新增掌握证据；完成后将根据单个查询、列表查询、分页转换和错误分支的实际表现更新本复盘。
...
- [ ] 区分认证（你是谁）和授权（你能访问什么）。
- [x] 为公开、登录用户和管理员接口分别配置访问规则。
- [x] 使用 HTTP Basic 测试真实用户名和密码，而不只使用模拟用户。
- [x] 允许 `http://localhost:5173` 的跨域预检请求。
- [x] 通过实际请求确认公开 `200`、未登录或错误密码 `401`、权限不足 `403`。

本课最终行为正确，但这些 API 都是第一次接触，开始时无法从旧知识直接推导写法。曾把 `/public` 写成不包含 Controller 前缀的短路径，导致请求落入 `anyRequest().authenticated()` 并返回 `401`；在解释 `User`、`UserDetails` 和 `import` 时，也暴露出“类、接口、方法、外部依赖中的类型”还需要继续区分。后续新框架内容应先给出类型地图和最小可运行示例，再让你填写局部表达式，而不是要求凭空猜 API。

## Lesson 30 针对性训练

- [x] 在令牌中设置用户名、签发时间和 30 分钟后的过期时间。
- [ ] 沿着“账号密码 -> Authentication -> JWT 字符串”说明登录响应的数据流。
- [x] 通过测试验证 `Authorization: Bearer <token>` 能恢复 `frontend` 身份。
- [x] 验证没有令牌、令牌被篡改和账号密码错误都返回 `401`。
- [ ] 解释 JWT 已签名不等于内容加密，前端不应把秘密数据放进令牌。

本课最终实现和三个测试都正确，但完成过程需要把每个 API 拆成单独一步。曾在 `claimsBuilder.build()` 之后才设置 `subject` 和 `expiresAt`，也曾创建 `JwsHeader` 却没有用变量接住；说明“Builder 先收集字段、最后生成对象”和“方法返回值要成为下一步输入”仍需反复练习。第一次验收还漏删一个 `@Disabled`，以后要在运行测试后主动确认 `Skipped: 0`。
...
- [x] 使用 `Optional.orElseThrow()` 把缺失商品转换为明确业务异常。
- [x] 在任何写操作之前完成库存校验。
- [ ] 沿着 `CreateOrderRequest -> ProductEntity -> PurchaseOrderEntity -> OrderResponse` 说明类型变化。
- [ ] 解释 `@Transactional` 如何让扣库存和保存订单保持一致。
- [x] 完成公开商品、JWT 登录和 Bearer 下单的完整请求流程。

本课最终实现了商品查询、库存校验、扣减库存、金额计算、订单持久化和真实响应，三个业务测试均已启用。过程中 TODO 2 曾重复调用 Repository 查询已经取得的商品；TODO 3 第一次把扣减后的库存 `3` 当成购买数量 `2` 保存，而原测试只检查订单条数，没有检查订单实体字段，因此出现了测试全绿但数据语义错误。修正后增加了持久化订单的商品、数量和总价断言。下一阶段的重点不是继续堆叠框架，而是先画清类型流和字段含义，再独立完成一个小型后端功能。
...
- 能生成带用户名和过期时间的 JWT，并通过 Bearer 请求恢复用户身份。
