...
> 复盘日期：2026-09-01
>
> 当前进度：已完成 Lesson 01 - Lesson 29，Lesson 30 进行中
>
> 当前阶段：已经能在提示下配置 Spring Security 路由、内存用户与 CORS，开始学习 JWT 如何携带用户身份
...
- [ ] 说出 JWT 的 `subject`、签发时间和过期时间分别表达什么。
- [ ] 沿着“账号密码 -> Authentication -> JWT 字符串”说明登录响应的数据流。
- [ ] 使用 `Authorization: Bearer <token>` 请求受保护接口。
- [ ] 区分没有令牌、令牌被篡改和账号密码错误三个失败场景。
...
达到这些标准后，再进入 OpenAPI 与 Docker 时，就能把安全接口文档化并放入可重复启动的运行环境。
